...
class ep_process:
	...
	def __init__(self, ip, port, building_path, weather, eplus_path=None):
		eplus_script = r'C:/EnergyPlusV8-8-0/RunEPlus'
		# eplus_script = r'C:/EnergyPlusV8-8-0/energyplus.exe'
		log_file = open(building_path +'/'+ 'epluslog.txt', 'w')
		...
		idf_file_path = os.path.abspath(os.path.join(building_path, '../..'))
...
def set_bcvtb_home():
	# path = r'C:/BCVTB'		# OK in local PC
	path= os.path.join(os.getcwd(), 'pyEp', 'bcvtb')	# OK as working stand-alone
	# print(path)
